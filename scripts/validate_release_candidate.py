...
def main():
    ...
    if rc["version"]!="1.0.0-rc.4": errors.append("Unexpected RC version")
    if rc["package_version"]!="0.44.2": errors.append("Unexpected package version")
    ...
    if project["project"]["project_version"]!="0.44.2": errors.append("project.yaml version mismatch")
    if project["project"]["plan_step_completed"]!=44: errors.append("project.yaml step mismatch")
    if project["architecture"].get("release_candidate")!="1.0.0-rc.4": errors.append("project.yaml RC mismatch")
    if "1.0.0-rc.4" not in (ROOT/"README.md").read_text(encoding="utf-8"): errors.append("README missing RC version")
    if errors:
        print("FAILED")
        for e in errors: print("-",e)
        return 1
    print("OK")
    print("Release candidate: 1.0.0-rc.4")

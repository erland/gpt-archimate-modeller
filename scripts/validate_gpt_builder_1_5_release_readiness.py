...
def main():
    ...
    if status.get("state",{}).get("overall")!="pass":
        errors.append("migration overall status must be pass")
    ...
    if contract.get("runtime_policy",{}).get("active")!=expected:
        errors.append("normalized contract active runtime set mismatch")
    ...
    if plugin.get("status")!="not_active" or plugin.get("compatibility")!="reduced":
        errors.append("OpenAI Plugin must remain not_active/reduced")
    ...
    for path in ["README.md","STATUS.md","docs/gpt-builder-1.5-runtime-migration.md"]:
        ...
        if "GPT Byggaren 1.5" not in text:
            errors.append(f"{path} missing GPT Builder 1.5 documentation")
